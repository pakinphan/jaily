...
app.post("/register", (req,res) => {
  ...
  if (newuser_username && newuser_password) {
    bcrypt.genSalt(10, function(err, salt) {
      ...
      bcrypt.hash(newuser_password, salt, function(err, hash) { 
        connection.query(
          "INSERT INTO user_info SET user_username = ?, user_password = ?, user_email = ?",
          [newuser_username, hash, newuser_email],  
          function(err) {
            if (err) {
    ...
    if (newuser_username && newuser_password) {
        connection.query(
            ...
            function(err, results) {
                ...
                if (results.length > 0) {
                    bcrypt.compare(newuser_password, results[0].user_password, function(err, result) {
                        if (result == true) {
                            req.session.loggedin    = true;
